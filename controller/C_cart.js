...
exports.UpdateQuantity = AsyncHandler(async (req, res, next) => {
  ...
  const data = await M_cart.findOne({ user: req.user._id })
  if (!data) next(new Error("there is no cart for user"))
  
  const ItemIndex = data.cartItems.findIndex(
    e => e._id.toString() === req.params.id)
  
  if (ItemIndex > -1) {
    const cartItem = data.cartItems[ItemIndex]
    cartItem.quantity = req.body.quantity
    data.cartItems[ItemIndex] = cartItem
  } else {
    return next( new Error("there is no item for this id"))
  }

  let totalPrice = 0;
  data.cartItems.forEach(e => totalPrice += e.quantity * e.price)
  data.totalCartPirce = totalPrice
  data.totalCartDiscount =undefined
  await data.save()
  res.status(200).json({numOfCartItems : data.cartItems.length , data })
})
